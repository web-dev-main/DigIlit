...
export default function QuantumMasterpiece() {
  ...
  const [formData, setFormData] = useState({
    name: '', email: '', type: 'Individual', interest: 'Virtual Assistants', message: ''
  });
  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      for (let i = 0; i < 5; i++) {
        ctx.beginPath();
        for (let x = 0; x < w; x += 5) {
          const y = h / 2 + Math.sin(x * 0.01 + time + i * 0.5) * 50 + Math.cos(x * 0.02 + time * 0.5) * 30;
  ...
  const handleSubmit = () => {
    if (formData.name && formData.email && formData.message) {
      setFormOpen(false);
      alert('🚀 Thank you! Our team will connect with you within 24 hours with a custom proposal.');
      setFormData({ name: '', email: '', type: 'Individual', interest: 'Virtual Assistants', message: '' });
    }
  };
  ...
  return (
    <div className="relative min-h-screen bg-black text-white overflow-x-hidden">
      ...
      <div className="fixed inset-0 bg-gradient-to-br from-purple-900/10 via-transparent to-cyan-900/10 pointer-events-none" style={{ zIndex: 1 }} />
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="ml-4 text-center">
                  <div className="text-3xl font-black tracking-wider">
                    DIG<span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-cyan-400">|</span>LIT
                  </div>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-6">
                <button 
                  ...
                  title="Search"
                >
                  <svg className="w-5 h-5 text-violet-300 group-hover:text-violet-200 transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                ...
                <div className="relative p-8 bg-gradient-to-br from-black/40 to-purple-900/20 backdrop-blur-2xl rounded-3xl border border-white/10 shadow-2xl hover:border-violet-500/30 transition-all">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="relative p-6 bg-gradient-to-br from-violet-600/20 to-purple-600/20 rounded-xl border border-white/10 overflow-hidden group/card hover:border-violet-500/40 transition-all cursor-pointer">
                      <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/5 to-transparent opacity-0 group-hover/card:opacity-100 transition-opacity" style={{ animation: 'shimmer 2s ease-in-out infinite' }} />
              ...
              {services.map((service, i) => (
                ...
                >
                  ...
                  <div className="relative">
                    <div className="text-5xl mb-4 group-hover:scale-110 transition-transform inline-block">{service.icon}</div>
                    ...
                    <div className={`inline-flex items-center gap-2 text-sm font-semibold bg-gradient-to-r ${service.color} bg-clip-text text-transparent group-hover:gap-3 transition-all`}>
                      Learn more 
        ...
        <footer className="py-12 px-8 border-t border-white/5 backdrop-blur-xl bg-black/20">
          <div className="max-w-7xl mx-auto">
            <div className="grid md:grid-cols-4 gap-8 mb-8">
              <div className="md:col-span-2">
                <div className="flex items-center gap-3 mb-4">
                  <div className="flex gap-1">
                    {[...Array(9)].map((_, i) => (
                      <div key={i} className="w-2 h-2 bg-gradient-to-r from-yellow-300 to-orange-400 rounded-full" style={{ animation: `atomShine ${2 + i * 0.3}s ease-in-out infinite`, animationDelay: `${i * 0.2}s` }} />
                    ))}
                  </div>
                  <div className="text-2xl font-black">DIG<span className="text-violet-400">|</span>LIT</div>
                </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {menuItems.map(item => (
                    <button key={item} onClick={() => setActiveSection(item.toLowerCase())} className="block text-gray-400 hover:text-violet-400 transition-colors text-sm">
                      {item}
                    </button>
                  ))}
                </div>
              ...
              <div>
                ...
                <button onClick={() => setFormOpen(true)} className="w-full px-4 py-2 bg-gradient-to-r from-violet-600 to-purple-600 rounded-lg font-semibold text-sm hover:scale-105 transition-transform">
                  Get in Touch
                </button>
              </div>
            </div>
            ...
            <div className="pt-8 border-t border-white/5 text-center text-sm text-gray-500">
              © {new Date().getFullYear()} Dig|lit. All rights reserved. Empowering enterprises with human intelligence and AI innovation.
            </div>
          </div>
        </footer>
      </div>
      ...
      {formOpen && (
        <div className="fixed inset-0 flex items-center justify-center p-4" style={{ zIndex: 50 }}>
          ...
          <div className="relative w-full max-w-2xl bg-gradient-to-br from-black/90 to-purple-900/30 backdrop-blur-2xl rounded-3xl border border-white/20 shadow-2xl p-8 animate-in">
            ...
            <p className="text-gray-400 mb-8">Tell us about your needs and we'll craft the perfect solution for you.</p>
